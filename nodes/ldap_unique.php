...
if ($CUDPerson) {
  ...
  $output .= "<a href=\"./index.php?n=person_unique&cudid=" . $CUDPerson[0]['cudid'] . "\">";
...
<div class="row">
  ...
  <?php
  if (in_array($ldapPerson->useraccountcontrol, array("514", "546"))) {
	$class = "text-bg-danger";
  } elseif ($ldapPerson->useraccountcontrol == "512") {
	...
	<div class="card">
	  ...
	  <div class="card-body">
		...
		foreach ($ldapPerson->memberof AS $memberOf) {
		  $url = "./index.php?n=ldap_all&filter=group&ldap_search=" . urlencode($memberOf);
...
<div class="row">
  <?php
  printArray($ldapPerson);
  $searchArray = array($ldapPerson->samaccountname, $CUDPerson->cudid);
  $searchLogs = $logsClass->allSearch($ldapPerson->samaccountname);
  echo $logsClass->makeTable($searchLogs);
  ?>
</div>
<?php } ?>
